...
# CONFIG
# ================================
DATA_PATH = "data"  # Folder with ephemeris_YYYY.csv and summary_YYYY.csv
...
def load_ephemeris(date):
    ...
    if not os.path.exists(file_path):
        st.error(f"No ephemeris file found for {year}")
        return pd.DataFrame()
...
def load_summary(year):
    ...
    if not os.path.exists(file_path):
        st.error(f"No summary file found for {year}")
        return pd.DataFrame()
...
st.title("🔮 Astro Market Dashboard (2025–2030)")
...
with tabs[0]:
    ...
    date = st.date_input("Select Date", datetime(2025, 8, 10), min_value=datetime(2025, 1, 1), max_value=datetime(2030, 12, 31))
    ...
    if not df.empty:
        ...
        today_df = today_df[today_df["Symbol"].isin(symbols)]
        st.dataframe(today_df[["DateTime", "Symbol", "Trend", "Events"]])
...
with tabs[1]:
    ...
    date = st.date_input("Select Date for Intraday", datetime(2025, 8, 10), min_value=datetime(2025, 1, 1), max_value=datetime(2030, 12, 31), key="intraday_date")
...
with tabs[2]:
    ...
    date = st.date_input("Select Date for Watchlist", datetime(2025, 8, 10), min_value=datetime(2025, 1, 1), max_value=datetime(2030, 12, 31), key="watchlist_date")
...
with tabs[3]:
    ...
    date = st.date_input("Select Date for Upcoming Transit", datetime(2025, 8, 10), min_value=datetime(2025, 1, 1), max_value=datetime(2030, 12, 31), key="upcoming_date")
